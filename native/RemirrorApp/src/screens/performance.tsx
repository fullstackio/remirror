...
import { Button, NativeSyntheticEvent, SafeAreaView, ScrollView, StyleSheet, Text } from 'react-native';
import { WebView, WebViewMessage, WebViewProgressEvent } from 'react-native-webview';
...
export class PerformanceScreen extends Component<{}, State> {
  ...
  private readonly onPress = () => {
    ...
    this.setState({ num: 0, started: new Date() }, () => {
      ...
      this.webView.current!.postMessage(String(this.state.num));
      console.log('posted message');
    });
  };

  private readonly onMessage = (event: NativeSyntheticEvent<WebViewMessage>) => {
    ...
    if (num >= MAX) {
      this.webView.current!.postMessage('STOP');
      return;
    }
    this.setState({ num, duration: Date.now() - this.state.started.getTime(), message: '' }, () =>
      this.webView.current!.postMessage(String(this.state.num + 1)),
    );
  };

  private readonly onLoadProgress = (e: NativeSyntheticEvent<WebViewProgressEvent>) =>
    ...
    return (
      <SafeAreaView style={styles.container}>
        <ScrollView>
          ...
          <WebView
            ref={this.webView}
            source={require('../../resources/file.html')}
