...
export const isEnterprise = () =>
  location.hostname !== 'github.com' && location.hostname !== 'gist.github.com';
...
export const isUserProfile = () =>
  Boolean(getCleanPathname()) &&
  !isGist() &&
  !isReserved(getCleanPathname()) &&
  !getCleanPathname().includes('/');
...
export const getPageType = (): GitHubPageType => {
  if (isRepo()) {
    return GitHubPageType.Repo;
  } else if (isUserProfile()) {
    return GitHubPageType.UserProfile;
  } else {
    return GitHubPageType.Unknown;
  }
};
