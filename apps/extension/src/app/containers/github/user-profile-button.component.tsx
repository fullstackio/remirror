...
export class UserProfileButton extends PureComponent<Props, State> {
  public state: State = {
    pledged: false,
    pledges: 0,
    tipped: false,
    tips: 0,
    element: createInjectedElement('div'),
  };
  ...
  constructor(props: Props) {
    super(props);
    this.container = this.props.elements.userProfile;
  }
  ...
  public componentDidUpdate(prevProps: InjectedContentProps) {
    const { userProfile } = this.props.elements;
    if (prevProps.elements.userProfile !== userProfile) {
      if (userProfile) {
        this.recreateFragment(userProfile);
      }
    }
  }
  ...
  private recreateFragment(newContainer: Element) {
    ...
    const element = createInjectedElement('div');
