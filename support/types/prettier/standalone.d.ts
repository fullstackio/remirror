import { check, CursorOptions, CursorResult, Options, Plugin } from 'prettier';

type Omit<GType, GKeys extends keyof GType> = Pick<GType, Exclude<keyof GType, GKeys>>;

type MakeRequired<GType extends {}, GKeys extends keyof GType> = Omit<GType, GKeys> &
  { [P in GKeys]-?: GType[P] };

type MR<GType extends { plugins: any; parser: any }> = MakeRequired<GType, 'plugins' | 'parser'>;
 ...
 * `-> { formatted: '1;\n', cursorOffset: 1 }`
 */
export function formatWithCursor(source: string, options: MR<CursorOptions>): CursorResult;
